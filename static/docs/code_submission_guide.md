...
- **返回值**：无。
- **被调用时机**：游戏开始时，由服务端分配玩家实例编号时调用, index 参数由服务端填充。
- **使用建议**：
...
- **返回值**：无。
- **被调用时机**：分配角色后立即调用， role_type 参数由服务端填充。
- **使用建议**：
...
5. **发言/移动轮次**：按顺序调用 `say`，广播每段发言并通过 `pass_message` 通知其他玩家。
